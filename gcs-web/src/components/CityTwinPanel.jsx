...
export default function CityTwinPanel() {
  ...
  const [simFormSuccess, setSimFormSuccess] = useState(null)
  ...
  const [markerSuccess, setMarkerSuccess] = useState(null)
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: '1 1 420px', minWidth: 360, display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div style={{ maxHeight: 200, overflowY: 'auto' }}>
              {models.length === 0 ? (
                ...
                models.map((m, i) => {
                  ...
                  return (
                    <div key={mid != null ? mid : `model-${i}`} style={{ padding: '6px 10px', borderBottom: '1px solid var(--line-2)', borderLeft: '3px solid var(--cyan)' }}>
            ...
            <div style={{ maxHeight: 160, overflowY: 'auto' }}>
              {markers.length === 0 ? (
                ...
                markers.map((m, i) => {
                  ...
                  return (
                    <div key={pick(m, 'id', 'markerId') ?? `marker-${i}`} style={{ padding: '4px 10px', borderBottom: '1px solid var(--line-2)', borderLeft: `3px solid ${meta.color}` }}>
          ...
          <div style={{ ...cardStyle, padding: 10 }}>
            ...
            {markerSuccess && <div style={{ color: 'var(--ok)', fontSize: 10, marginBottom: 4 }}>✓ {markerSuccess}</div>}
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              <div style={{ flex: '1 1 100px' }}>
                <select aria-label="标绘类型" value={markerForm.type} onChange={(e) => updateMarkerForm('type', e.target.value)} style={modalInputStyle}>
                  {Object.entries(MARKER_TYPE_META).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
                </select>
              </div>
              <div style={{ flex: '1 1 100px' }}>
                <input type="number" step="any" aria-label="标绘纬度" placeholder="纬度" value={markerForm.lat} onChange={(e) => updateMarkerForm('lat', e.target.value)} style={modalInputStyle} />
              </div>
              <div style={{ flex: '1 1 100px' }}>
                <input type="number" step="any" aria-label="标绘经度" placeholder="经度" value={markerForm.lon} onChange={(e) => updateMarkerForm('lon', e.target.value)} style={modalInputStyle} />
              </div>
              <div style={{ flex: '1 1 120px' }}>
                <input type="text" aria-label="标绘标签" placeholder="标签" value={markerForm.label} onChange={(e) => updateMarkerForm('label', e.target.value)} style={modalInputStyle} />
              </div>
        ...
        <div style={{ flex: '1 1 420px', minWidth: 360, display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <div style={{ ...cardStyle, padding: 10 }}>
            ...
            {simFormSuccess && <div style={{ color: 'var(--ok)', fontSize: 10, marginBottom: 4 }}>✓ {simFormSuccess}</div>}
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              <div style={{ flex: '1 1 120px' }}>
                <div style={labelStyle}>灾害类型</div>
                <select aria-label="灾害类型" value={simForm.type} onChange={(e) => updateSimForm('type', e.target.value)} style={modalInputStyle}>
                  {DISASTER_TYPES.map((t) => <option key={t.key} value={t.key}>{t.label}</option>)}
                </select>
              </div>
              <div style={{ flex: '1 1 100px' }}>
                <div style={labelStyle}>中心纬度</div>
                <input type="number" step="any" aria-label="中心纬度" value={simForm.centerLat} onChange={(e) => updateSimForm('centerLat', e.target.value)} style={modalInputStyle} placeholder="纬度" />
              </div>
              <div style={{ flex: '1 1 100px' }}>
                <div style={labelStyle}>中心经度</div>
                <input type="number" step="any" aria-label="中心经度" value={simForm.centerLon} onChange={(e) => updateSimForm('centerLon', e.target.value)} style={modalInputStyle} placeholder="经度" />
              </div>
              <div style={{ flex: '1 1 80px' }}>
                <div style={labelStyle}>半径 (km)</div>
                <input type="number" step="any" aria-label="影响半径（公里）" value={simForm.radiusKm} onChange={(e) => updateSimForm('radiusKm', e.target.value)} style={modalInputStyle} placeholder="km" />
              </div>
              <div style={{ flex: '1 1 80px' }}>
                <div style={labelStyle}>{TYPES_REQUIRING_DURATION.includes(simForm.type) ? '时长 (min) *' : '时长 (min)'}</div>
                <input type="number" aria-label="模拟时长（分钟）" value={simForm.durationMin} onChange={(e) => updateSimForm('durationMin', e.target.value)} style={modalInputStyle} placeholder={TYPES_REQUIRING_DURATION.includes(simForm.type) ? '必填' : '可选'} />
              </div>
              {/* 灾害类型特定额外参数 */}
              {(DISASTER_EXTRA_PARAMS[simForm.type] || []).map((param) => (
                <div key={param.key} style={{ flex: '1 1 100px' }}>
                  <div style={labelStyle}>{param.label} *</div>
                  <input type="number" step="any" aria-label={param.label} value={simForm[param.key]} onChange={(e) => updateSimForm(param.key, e.target.value)} style={modalInputStyle} placeholder={param.placeholder} />
                </div>
              ))}
            </div>
